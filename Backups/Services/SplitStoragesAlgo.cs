...
namespace Backups
{
    public class SplitStoragesAlgo : IAlgorithm
    {
        ...
        public RestorePoint MakePoint(List<JobObject> list)
        {
            ...
            {
                string zipFile = Directory.GetParent(Environment.CurrentDirectory)?.Parent?.Parent?.FullName + "/Backups/BackupWorkFiles/ZipFile" + Convert.ToString(_counterDirectories);
